...
$_SESSION['idpengguna'] = 'admin'; header('location: admin/');
} else {
gagal();
}
...
$_SESSION['idstaff'] = $idstaff; header('location: staff/');
} else {
gagal();
}
...
$stmt->fetch();
if (password_verify($katalaluan, $kata)) {
$_SESSION['idcustomer'] = $idcustomer; header('location: customer/');
} else {
gagal();
}
} else {
...
?>
<script>
alert('Maaf, ID pengguna/kata laluan salah.'); window.location = './';
</script>
<?php
}
